...
export class GreeterWizard {
     ...
     @On('text')
     @WizardStep(2)
     async onName(@Ctx() ctx: WizardContext, @Message() msg: { text: string }): Promise<string> {
     ...
     @On('text')
     @WizardStep(3)
     async onLocation(@Ctx() ctx: WizardContext & { wizard: { state: { name: string } } }, @Message() msg: { text: string }): Promise<string> {
